...
pig.core.updateHero = function(modifier) {
	...
	if (37 in pig.session.keysDown || 65 in pig.session.keysDown) { 
		...
		} else if (hasLeftCollision(pig.session.hero).type === 'tropyh') {
			var nextLevel = new pig.objects.level3();
			nextLevel.imageObject = new Image();
			nextLevel.imageObject.addEventListener('load', (function(nextLevel) {
				return function() {
					pig.session.hero.x = pig.session.canvas.width / 2;
	...
	for (i = pig.session.projectiles.length; i--;) {
		...
		if (pig.session.projectiles[i].isHero) {
			for (n = pig.session.level.monsters.length; n--;) {
				if (Math.abs(pig.session.level.monsters[n].x + 23 - pig.session.projectiles[i].x) < 23 &&
					Math.abs(pig.session.level.monsters[n].y + 23 - pig.session.projectiles[i].y) < 23) {
					pig.session.projectiles.splice(i, 1);
					pig.session.level.monsters[n].health -= 23;
					// if moster is dead
			...
			if (Math.abs(pig.session.hero.x + 23 - pig.session.projectiles[i].x) < 23 &&
				Math.abs(pig.session.hero.y + 23 - pig.session.projectiles[i].y) < 23) {
				pig.session.projectiles.splice(i, 1);
				pig.session.hero.health -= 18;
				// if hero is dead
