...
pig.gfx.sprites = {
	...
	trophy: new pig.objects.sprite('images/trophy.png'),	
...
pig.gfx.renderFrame = function renderFrame() {
	...
	if (pig.session.dead) {
		pig.gfx.drawStopText('lol u died');
	} else if (pig.session.paused) {
		pig.gfx.drawStopText('cya soon');
	}
...
pig.gfx.drawMonsters = function drawMonsters() {
	...
	for (i = pig.session.level.monsters.length; i--;) {
		...
		if (pig.session.level.monsters[i].message) {
			...
			pig.session.ctx.fillStyle = 'rgb(255,255,0)';
			pig.session.ctx.fillRect(
...
pig.gfx.drawProjectiles = function drawProjectiles() {
	...
	for (i = pig.session.projectiles.length; i--;) {
		pig.session.ctx.fillStyle = pig.session.projectiles[i].isHero ? 'rgb(0,100,160)' : 'rgb(123,255,0)';
		pig.session.ctx.fillRect(
...
var m_context = m_canvas.getContext('2d');
	...
	m_context.fillText('KillCount: ' + pig.session.score, 0, 0);
